...
上面的例子介绍了很多Turtle语言的特性，如： [@base](/base ) and Relative IRIs, [@prefix](/prefix ) and prefixed names, 由';分开的'predicate 列表，由','分开的objects列表，象征(token)项a以及字符串。
...
IRIs 可以写为相对的或绝对的IRIs 或前缀名称。相对的或绝对的IRIs 被'<' 和 '>'包围，可能包含数字转义序列(如下所述)。如：[http://example.org/#green-goblin](http://example.org/#green-goblin)。
...
示例中的项 'a'是处于 predicate位置上的，用来表示 [http://www.w3.org/1999/02/22-rdf-syntax-ns#type](http://www.w3.org/1999/02/22-rdf-syntax-ns#type) 这个 IRI。
...
![](/img/in-post/turtle1.1/numbers.png)![](/img/yuque/计算机/杂/assets/RDF_1.1_Turtle_中文翻译-1.png)
...
RDF空白节点表示为\_：后跟空白节点标签，后者是一系列名称字符。标签中的字符构建在PN\_CHARS\_BASE之上，放宽限制如下：

  

-   字符\_和数字可以出现在空白节点标签中的任何位置。
...
识别Turtle语言的IRI是：http：[//www.w3.org/ns/formats/Turtle。](//www.w3.org/ns/formats/Turtle%E3%80%82)
...
![](/img/yuque/计算机/杂/assets/RDF_1.1_Turtle_中文翻译-2.png)
...
![](/img/yuque/计算机/杂/assets/RDF_1.1_Turtle_中文翻译-3.png)
...
![](/img/yuque/计算机/杂/assets/RDF_1.1_Turtle_中文翻译-4.png)
...
![](/img/yuque/计算机/杂/assets/RDF_1.1_Turtle_中文翻译-5.png)

  

![](/img/in-post/turtle1.1/grammer_4.png)![](/img/yuque/计算机/杂/assets/RDF_1.1_Turtle_中文翻译-6.png)
...
-   Map [prefix - > IRI]名称空间 - prefixID生成中的第二个和第三个规则参数（PNAME\_NS和IRIREF）为前缀（PNAME\_NS）分配名称空间名称（IRIREF）。 在prefixID生成之外，所有PNAME\_NS都将替换为命名空间。请注意，根据PNAME\_NS生成，前缀可能是空字符串：（PN\_PREFIX）?":"。

-   Map [string - > blank node] bnodeLabels - 从字符串到空白节点的映射。

-   RDF\_Term curSubject - curSubject绑定到subject production。

-   RDF\_Term curPredicate - curPredicate与动词生成绑定。RDF\_Term curPredicate - curPredicate与动词生成绑定。 如果token匹配为“a”，则curPredicate绑定到IRI http://www.w3.org/1999/02/22-rdf-syntax-ns#type。
...
![](/img/in-post/turtle1.1/grammer_6.png)![](/img/yuque/计算机/杂/assets/RDF_1.1_Turtle_中文翻译-7.png)
...
-   分配IRI [http://www.w3.org/People/Eric/ericP-foaf.rdf#ericP](http://www.w3.org/People/Eric/ericP-foaf.rdf#ericP) 给 curSubject。
...
-   返回一个RDF三元组：<... rdf＃ericP> <... / knows> [http://getopenid.com/amyvdh](http://getopenid.com/amyvdh)。
