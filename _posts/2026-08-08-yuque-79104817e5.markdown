...
![](/img/yuque/计算机/深度学习/深度学习基础/assets/10._Attention_基础-1.gif)

可以看到，在 decoder 每个输出时， attention 将重点放在几个相关的输入上。 Attention 的这种关注是通过对不同的输入分配不同的权重实现的。

# Attention 的原理

![](/img/yuque/计算机/深度学习/深度学习基础/assets/10._Attention_基础-2.png)

上图是一个比较经典的 attention 图，其中 $\overleftarrow{h_{.}}$ 和 $\overrightarrow{h_{.}}$ 是隐藏状态的输出，现在我们将做 decoder计算 decoder 状态 $s_{t}$：

-   计算每一个输入位置 j 与 当前输出位置的关联性$e_{t,j} = align(s_{t-1}, h_{j})$，写成向量形式为：$\overrightarrow{e{t}} = (align(s{t-1}, h{1}),\dots,align(s{t-1}, h{T}))$。$e_{ij}$表示一个对齐模型，计算方式有很多种，不同的计算方式代表不同的 attention 模型，最简单的且最常用的对齐模型是矩阵相乘，常见的对齐方式为：
...
![](/img/yuque/计算机/深度学习/深度学习基础/assets/10._Attention_基础-3.jpeg)
...
![](/img/yuque/计算机/深度学习/深度学习基础/assets/10._Attention_基础-4.jpeg)
...
其中 S 表示 encoder 端的句子长度，$v_{p}$和 $w_{p}$是模型参数。得到 $p_{t}$ 后，$c_{t}$的计算将值关注窗口 $[p{t}-D, p{t}+D]$ 内的2D+1 个 encoder 输入。对齐向量 $a_{t}$的计算公式为：
...
![](/img/yuque/计算机/深度学习/深度学习基础/assets/10._Attention_基础-5.gif)
...
![](/img/yuque/计算机/深度学习/深度学习基础/assets/10._Attention_基础-6.jpeg)
...
![](/img/yuque/计算机/深度学习/深度学习基础/assets/10._Attention_基础-7.png)
...
![](/img/yuque/计算机/深度学习/深度学习基础/assets/10._Attention_基础-8.png)
...
![](/img/yuque/计算机/深度学习/深度学习基础/assets/10._Attention_基础-9.png)
...
2.  [https://arxiv.org/abs/1706.03762](https://arxiv.org/abs/1706.03762)
3.  [《Attention is All You Need》浅读（简介+代码）](https://spaces.ac.cn/archives/4765/comment-page-1)
{% endraw %}
