...
先说一个有意思的事，ELMO 这个名字来源于芝麻街中的一个角色名，后续的很多模型都顺着这个叫，比如 BERT。最搞笑的是 ENRIE, 百度和清华两个组做的工作同时叫这一个名字 = =，

ELMO 采用两阶段模式，第一阶段根据自回归任务和大量数据进行训练，得到模型。第二阶段即使用阶段通过对隐层表示和输入进行线性组合得到新的更好的表示作为目标模型的输入。

预训练阶段采用正向 + 反向 LSTM 组合的 biLM 形式，分别获取当前词的上文和下文信息。其中正向 LSTM 通过给定上文预测当前词，反向 LSTM 则是给定下文预测当前词。给定一个长度为 N 的序列 $$ t_{1}, t_{2}, \dots, t_{N}$$, 用公式表达就是
...
更细节来说， biLM 是层数为2的 biLSTM，隐藏单元大小为 4096，projections 维度 512， 第一层和第二层之间有残差连接。下图是 elmo 的网络结构
...
ELMo 适合短文本任务，ELMo迁移到下游网络中， 一个是答案较短的数据集，提升有3-4个点， 一个答案较长的数据集，提升只有0.5 左右。
