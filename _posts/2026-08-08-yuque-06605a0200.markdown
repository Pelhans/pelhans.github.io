...
![image.png](/img/yuque/计算机/大模型/assets/预训练模型总结(BERT_量级)-1.png)
...
一定程度的 dropout 可以提升模型的泛化能力。除此之外，还可以采用 L2 正则项$\lambda ||w||_{2}^{2}$。大的 $\lambda$ 将使得权重趋同，小的 $\lambda$ 使得 同一层的权值多样。实验中发现，对于大多数任务，小的 $\lambda$ 效果较好，如下图所示：

![image.png](/img/yuque/计算机/大模型/assets/预训练模型总结(BERT_量级)-2.png)
...
![image.png](/img/yuque/计算机/大模型/assets/预训练模型总结(BERT_量级)-3.png)
...
![image.png](/img/yuque/计算机/大模型/assets/预训练模型总结(BERT_量级)-4.png)
...
![image.png](/img/yuque/计算机/大模型/assets/预训练模型总结(BERT_量级)-5.png)

最终 ELMO 在各任务上的表现如下图所示：

![image.png](/img/yuque/计算机/大模型/assets/预训练模型总结(BERT_量级)-6.png)
...
![image.png](/img/yuque/计算机/大模型/assets/预训练模型总结(BERT_量级)-7.png)

下图右侧是对各种下游任务的使用方式:

![image.png](/img/yuque/计算机/大模型/assets/预训练模型总结(BERT_量级)-8.png)
...
在迁移学习中，除非特别说明，否则将用来自无监督预训练的超参数设置。dropout 率为 0.1。对于大多数任务，学习率用6.25e-5， batch\_size 用32。 收敛很快，epoch 一般用 3 就行。使用线性学习率衰减，warmup over 训练的0.2%。$\lambda$ 设置为0.5。

实验方面，论文采用包括自然语言推理、问答、语义相似度和文本分类在内的几种代表性任务。其中一些任务作为最近发布的GLUE multi task benchmark的一部分提供。对应的实验结果如下图所示：

![image.png](/img/yuque/计算机/大模型/assets/预训练模型总结(BERT_量级)-9.png)

![](/img/in-post/pretrain_model/gpt_nli.JPG)![image.png](/img/yuque/计算机/大模型/assets/预训练模型总结(BERT_量级)-10.png)

![image.png](/img/yuque/计算机/大模型/assets/预训练模型总结(BERT_量级)-11.png)
...
![image.png](/img/yuque/计算机/大模型/assets/预训练模型总结(BERT_量级)-12.png)
...
![image.png](/img/yuque/计算机/大模型/assets/预训练模型总结(BERT_量级)-13.png)
...
![image.png](/img/yuque/计算机/大模型/assets/预训练模型总结(BERT_量级)-14.png)

至于更大的模型，这个是真的无敌， 48 层的网络，最后还告诉你这还有点欠拟合，也就是说网络还可以更深，更大。

![image.png](/img/yuque/计算机/大模型/assets/预训练模型总结(BERT_量级)-15.png)
...
![image.png](/img/yuque/计算机/大模型/assets/预训练模型总结(BERT_量级)-16.png)
...
![image.png](/img/yuque/计算机/大模型/assets/预训练模型总结(BERT_量级)-17.png)
...
![image.png](/img/yuque/计算机/大模型/assets/预训练模型总结(BERT_量级)-18.png)
...
![image.png](/img/yuque/计算机/大模型/assets/预训练模型总结(BERT_量级)-19.png)
...
![image.png](/img/yuque/计算机/大模型/assets/预训练模型总结(BERT_量级)-20.png)
...
![image.png](/img/yuque/计算机/大模型/assets/预训练模型总结(BERT_量级)-21.png)
...
![image.png](/img/yuque/计算机/大模型/assets/预训练模型总结(BERT_量级)-22.png)
...
![image.png](/img/yuque/计算机/大模型/assets/预训练模型总结(BERT_量级)-23.png)

在特征的使用方面，是用 feature-based 的还是 finetune 的呢？下表给出答案--都行，用最后四层 concat 的方法和 finetune 的表现差不多：

![image.png](/img/yuque/计算机/大模型/assets/预训练模型总结(BERT_量级)-24.png)
...
![image.png](/img/yuque/计算机/大模型/assets/预训练模型总结(BERT_量级)-25.png)
...
-   Query stream: 提供了查询表达 query representation $G_{\Theta}(w{z<t}, zt)$ ，记做 $G_{zt}$ ，它仅编码了 $w_{zt}$ 的上下文及其位置  $z_{t}$，并未编码 $w_{zt}$ 本身。
...
![image.png](/img/yuque/计算机/大模型/assets/预训练模型总结(BERT_量级)-26.png)
...
![image.png](/img/yuque/计算机/大模型/assets/预训练模型总结(BERT_量级)-27.png)
...
![image.png](/img/yuque/计算机/大模型/assets/预训练模型总结(BERT_量级)-28.png)
...
![image.png](/img/yuque/计算机/大模型/assets/预训练模型总结(BERT_量级)-29.png)
...
![image.png](/img/yuque/计算机/大模型/assets/预训练模型总结(BERT_量级)-30.png)
...
![image.png](/img/yuque/计算机/大模型/assets/预训练模型总结(BERT_量级)-31.png)
...
![image.png](/img/yuque/计算机/大模型/assets/预训练模型总结(BERT_量级)-32.png)
...
![image.png](/img/yuque/计算机/大模型/assets/预训练模型总结(BERT_量级)-33.png)
...
![image.png](/img/yuque/计算机/大模型/assets/预训练模型总结(BERT_量级)-34.png)
...
![image.png](/img/yuque/计算机/大模型/assets/预训练模型总结(BERT_量级)-35.png)
...
![image.png](/img/yuque/计算机/大模型/assets/预训练模型总结(BERT_量级)-36.png)
...
![image.png](/img/yuque/计算机/大模型/assets/预训练模型总结(BERT_量级)-37.png)
...
![image.png](/img/yuque/计算机/大模型/assets/预训练模型总结(BERT_量级)-38.png)
...
1.  [清华 PLM 必读论文](https://github.com/thunlp/PLMpapers)
2.  Unsupervised Pretraining for Sequence to Sequence Learning. Prajit Ramachandran, Peter J. Liu, Quoc V. Le. EMNLP 2017.
3.  Deep contextualized word representations. Matthew E. Peters, Mark Neumann, Mohit Iyyer, Matt Gardner, Christopher Clark, Kenton Lee and Luke Zettlemoyer. NAACL 2018.
4.  Universal Language Model Fine-tuning for Text Classification. Jeremy Howard and Sebastian Ruder. ACL 2018.
5.  Improving Language Understanding by Generative Pre-Training. Alec Radford, Karthik Narasimhan, Tim Salimans and Ilya Sutskever.
6.  BERT: Pre-training of Deep Bidirectional Transformers for Language Understanding. Jacob Devlin, Ming-Wei Chang, Kenton Lee and Kristina Toutanova. NAACL 2019
7.  Language Models are Unsupervised Multitask Learners. Alec Radford, Jeffrey Wu, Rewon Child, David Luan, Dario Amodei and Ilya Sutskever. Preprint.
8.  ERNIE: Enhanced Language Representation with Informative Entities. Zhengyan Zhang, Xu Han, Zhiyuan Liu, Xin Jiang, Maosong Sun and Qun Liu. ACL 2019.
9.  ERNIE: Enhanced Representation through Knowledge Integration. Yu Sun, Shuohuan Wang, Yukun Li, Shikun Feng, Xuyi Chen, Han Zhang, Xin Tian, Danxiang Zhu, Hao Tian and Hua Wu.
10.  Cross-lingual Language Model Pretraining. Guillaume Lample, Alexis Conneau. NeurIPS 2019.
11.  Multi-Task Deep Neural Networks for Natural Language Understanding. Xiaodong Liu, Pengcheng He, Weizhu Chen, Jianfeng Gao. ACL 2019.
...
13.  Unified Language Model Pre-training for Natural Language Understanding and Generation. Li Dong, Nan Yang, Wenhui Wang, Furu Wei, Xiaodong Liu, Yu Wang, Jianfeng Gao, Ming Zhou, Hsiao-Wuen Hon
14.  XLNet: Generalized Autoregressive Pretraining for Language Understanding. Zhilin Yang, Zihang Dai, Yiming Yang, Jaime Carbonell, Ruslan Salakhutdinov, Quoc V. Le. NeurIPS 2019
15.  RoBERTa: A Robustly Optimized BERT Pretraining Approach. Yinhan Liu, Myle Ott, Naman Goyal, Jingfei Du, Mandar Joshi, Danqi Chen, Omer Levy, Mike Lewis, Luke Zettlemoyer, Veselin Stoyanov
16.  SpanBERT: Improving Pre-training by Representing and Predicting Spans. Mandar Joshi, Danqi Chen, Yinhan Liu, Daniel S. Weld, Luke Zettlemoyer, Omer Levy
17.  K-BERT: Enabling Language Representation with Knowledge Graph. Weijie Liu, Peng Zhou, Zhe Zhao, Zhiruo Wang, Qi Ju, Haotang Deng, Ping Wang
18.  ERNIE 2.0: A Continual Pre-training Framework for Language Understanding. Yu Sun, Shuohuan Wang, Yukun Li, Shikun Feng, Hao Tian, Hua Wu, Haifeng Wang
19.  Pre-Training with Whole Word Masking for Chinese BERT. Yiming Cui, Wanxiang Che, Ting Liu, Bing Qin, Ziqing Yang, Shijin Wang, Guoping Hu. Preprint(Chinese-BERT-wwm)
20.  ELECTRA: Pre-training Text Encoders as Discriminators Rather Than Generators. Kevin Clark, Minh-Thang Luong, Quoc V. Le, Christopher D. Manning.
21.  Pre-training Tasks for Embedding-based Large-scale Retrieval. Wei-Cheng Chang, Felix X. Yu, Yin-Wen Chang, Yiming Yang, Sanjiv Kumar. ICLR 2020
22.  ALBERT: A Lite BERT for Self-supervised Learning of Language Representations. Zhenzhong Lan, Mingda Chen, Sebastian Goodman, Kevin Gimpel, Piyush Sharma, Radu Soricut. ICLR 2020.
23.  Q-BERT: Hessian Based Ultra Low Precision Quantization of BERT. Sheng Shen, Zhen Dong, Jiayu Ye, Linjian Ma, Zhewei Yao, Amir Gholami, Michael W. Mahoney, Kurt Keutzer
24.  TinyBERT: Distilling BERT for Natural Language Understanding. Xiaoqi Jiao, Yichun Yin, Lifeng Shang, Xin Jiang, Xiao Chen, Linlin Li, Fang Wang, Qun Liu. Preprint.
25.  How Contextual are Contextualized Word Representations? Comparing the Geometry of BERT, ELMo, and GPT-2 Embeddings. Kawin Ethayarajh. EMNLP 2019
26.  Are Sixteen Heads Really Better than One?. Paul Michel, Omer Levy, Graham Neubig.
27.  Patient Knowledge Distillation for BERT Model Compression
{% endraw %}
