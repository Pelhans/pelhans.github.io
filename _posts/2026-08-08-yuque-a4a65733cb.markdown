...
![image.png](/img/yuque/计算机/传统机器学习/assets/3._条件随机场_CRF-1.png)
...
这个局部范围要求为我们的分解一定要让 A 和B 不出现在同一因子中, 从而让属于这个图的所有可能的概率分布都满足条件独立的性质.

​  

因此需要团块的概念, 它的定义是图中节点的一个子集, 团块中的节点集合是全连接的.最大团快定义为将图中得任何一个其他节点加入到这个团块中都会破坏团的定义.现在我们可以将联合概率分布分解的因子定义为团块中变量的函数.为了方便,我们可以采用最大团块的函数而不失一般性,这是因为其他团块一定是最大团块的子集.
...
**势函数可以看作一种度量,它表示了局部变量的哪种配置优于其他的配置. E前面的负号表示能量越低的越可能出现。**
...
其中 $\beta{v}, \alpha{u,v}$ 表示系数. $t_{u,v}(u,v), s_{v}(v)$ 表示约束条件, 如CRF中的特征函数. 第一项考虑每一对节点之间的关系, 第二项考虑单个节点.
...
设 X 与 Y 是随机变量，$P(Y|X)$是在给定X 的条件下 Y 的条件概率分布， 若随机变量 Y 构成一个由无向图 G(V, E) 构成的马尔科夫随机场，即：
...
则称条件概率分布$P(Y|X)$为条件随机场。 这么说其实感觉称呼为条件马尔科夫随机场更合适。

​  

**条件随机场作为无向图**的一种, 具有多种形式, 不过最常见的还是链式CRF, 如下图所示:

​  

![image.png](/img/yuque/计算机/传统机器学习/assets/3._条件随机场_CRF-2.png)
...
![image.png](/img/yuque/计算机/传统机器学习/assets/3._条件随机场_CRF-3.png)
...
![](/img/in-post/ml_mianshi/crf_matrix.jpg)![image.png](/img/yuque/计算机/传统机器学习/assets/3._条件随机场_CRF-4.png)
...
![](/img/in-post/ml_mianshi/CRF_matrix.jpg)![image.png](/img/yuque/计算机/传统机器学习/assets/3._条件随机场_CRF-5.png)
...
$P(X|Y)$, 其中$Y_{i}$ 的取值集合为 $Y = \{y_{1}, y_{2}, \dots, y_{m}\}$, 给定观测序列 $\tilde{X} = \{x_{1}, x_{2}, \dots, x_{n}\}$, 给定标记序列 $\tilde{Y} = {\tilde{y}{1}, \tilde{y}{2}, \dots, \tilde{y}_{n}}$, 其中 $\tilde{y}_{i}\in Y$, 求:
...
似然函数要对整个数据集考虑，因此现在给定数据集 $D = \{(X{1}, Y{1}), (X{2}, Y{2}, \dots, (X{N}, Y{N}))\}$, 其中每一对$(X_{i}, Y_{i})$ 表示一个实际训练数据， $X_{i}$ 是长度为 $n_{i}$ 的观测序列， $Y_{i}$ 表示长度为 $n_{i}$ 的 标记序列。标记序列中的每个标记可能取值有 m 个。
...
![image.png](/img/yuque/计算机/传统机器学习/assets/3._条件随机场_CRF-6.png)
...
现在还有点问题就是参数太多了, 对应办法是加正则化项.L2 的 正则项是 $\sum_{k=1}{K}\frac{w_{k}{2}}{2\sigma^{2}}$, L1 的是$\alpha\sum_{k=1}^{K}|w_{k}|$
...
CRF 实际上是定义在时序数据上的对数线性模型, 因此CRF还可以看成序列化的logistic regression。这点可以从公式上来看, 我们将LR的概率公式变一下型,可以得到:
...
def viterbi_algorithm(transition_matrix, emission_matrix, initial_distribution, observations):
    """
    Viterbi Algorithm to find the most probable state sequence for a given observation sequence.

    :param transition_matrix: A 2D numpy array of shape (N, N) representing state transition probabilities.
    :param emission_matrix: A 2D numpy array of shape (N, M) representing emission probabilities.
    :param initial_distribution: A 1D numpy array of shape (N,) representing the initial state probabilities.
    :param observations: A list or numpy array of observed emissions.
    :return: The most probable state sequence as a list.
    """
    # Number of states
    ...
    # Initialization step
    V[:, 0] = np.log(initial_distribution) + np.log(emission_matrix[:, observations[0]])

    # Dynamic programming step
    for t in range(1, T):
        for s in range(num_states):
            prob = V[:, t-1] + np.log(transition_matrix[:, s]) + np.log(emission_matrix[s, observations[t]])
            backpointer[s, t] = np.argmax(prob)
            V[s, t] = np.max(prob)

    # Backtracking step
    best_path = np.zeros(T, dtype=int)
    best_path[-1] = np.argmax(V[:, T-1])
...
# Example usage:
# Define the parameters for the CRF model (for illustration purposes)
transition_matrix_example = np.array([[0.7, 0.3], [0.4, 0.6]])
emission_matrix_example = np.array([[0.1, 0.4, 0.5], [0.6, 0.3, 0.1]])
initial_distribution_example = np.array([0.6, 0.4])
observations_example = [0, 1, 2, 1, 0]  # The actual observations

# Call the Viterbi algorithm
state_sequence = viterbi_algorithm(transition_matrix_example,
                                   emission_matrix_example,
                                   initial_distribution_example,
