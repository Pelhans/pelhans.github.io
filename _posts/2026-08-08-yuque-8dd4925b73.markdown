...
![](/img/yuque/计算机/深度学习/深度学习基础/assets/7._激活函数-1.png)

![](/img/yuque/计算机/深度学习/深度学习基础/assets/7._激活函数-2.png)
...
它可以在二分类问题中，通过使用贝叶斯公式很自然的得到。首先我们有两个类比 C1 和C2，现在我们想计算 $p(C_{1}|x)$，那么根据贝叶斯公式有：

$$
p(C_{1}|x) = \frac{p(x|C_{1})*p(C_{1})}{p(x|C_{1})p(C_{1}) + p(x|C_{2})p(C_{2})}
$$

分子分母同时除以 $p(x|C_{1})p(C_{1})$，并令 $a = \ln\frac{p(x|C_{1})p(C_{1})}{p(x|C_{2})p(C_{2})}$，因此我们得到sigmoid公式：
...
![](/img/yuque/计算机/深度学习/深度学习基础/assets/7._激活函数-3.png)
...
![](/img/yuque/计算机/深度学习/深度学习基础/assets/7._激活函数-4.png)
...
![](/img/yuque/计算机/深度学习/深度学习基础/assets/7._激活函数-5.png)
...
其中 $\Phi$ 是标准正态累积分布函数（Cumulative Distribution Function，CDF）是**指正态分布的随机变量小于或等于某个特定值的概率。**

$$
\Phi(x) = \frac{1}{2}[1+erf(\frac{x}{\sqrt{2}})]
$$

其中，x 是随机变量的值，$\Phi(x)$是随机变量小于或等于 x 的概率，而 $\text{erf}$ 是误差函数（error function），它是高斯函数（正态分布的概率密度函数）的积分，并且可以用来表示正态分布的累积分布函数。

误差函数的表达式为：

$$
erf(x) = \frac{2}{\pi}\int_{0}^{x} e^{-t} dt
$$
...
![image.png](/img/yuque/计算机/深度学习/深度学习基础/assets/7._激活函数-6.png)

当输入x 较大时，$\Phi(x)$接近 1，此时函数像 RELU 的线性部分，当x 等于0 时是0，小于0 时，它的值特别小，像 leak RELU。

对于标准正态分布得情况,论文中提供了近似计算得数学公式:

$$
GELU(x) = 0.5\times(1 + tanh[\sqrt(\frac{2}{\pi})(x+0.044715x^{3})])
$$
...
2.  [深度学习中，使用relu存在梯度过大导致神经元“死亡”，怎么理解？](https://www.zhihu.com/question/67151971)
3.  [Hengkai Guo](https://www.zhihu.com/question/67366051/answer/262087707)
4.  [wiki](https://en.wikipedia.org/wiki/Activation_function)
{% endraw %}
