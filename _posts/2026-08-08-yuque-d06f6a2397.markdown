...
-   均匀随机：tf.initializers.random\_uniform(-0.1, 0.1)

-   正态分布：tf.initializaers.random\_normal(0, 1)，均值为0，方差为1的正态分布。

-   正态分布带截断：tf.initializers.truncated\_normal(0, 1)，生成均值为0，方差为1的正态分布，若产生随机数落到2$\sigma$外，则重新生成。

下面以均匀分布为例说明随机初始化的缺点。

均匀初始化即在一定范围内随机进行初始化,首先权重矩阵初始化公式为：
...
那个初始化公式怎么来的呢? 首先我们假设在[a,b] 内均匀分布,因此概率密度函数为 $f(x) = \frac{1}{b-a}$,当 $x\in [a,b]$, 否则为 0. 因为要对称, 所以 a = -b, 带入方差公式,消除 b 可得方差为:
...
![](/img/yuque/计算机/深度学习/深度学习基础/assets/4._神经网络中的权值初始化-1.png)

对于反向传播状态的梯度的方差是逐层增加的，换句话说是在反向传播过程中逐层递减的。如下图所示：

![](/img/yuque/计算机/深度学习/深度学习基础/assets/4._神经网络中的权值初始化-2.png)
...
![](/img/yuque/计算机/深度学习/深度学习基础/assets/4._神经网络中的权值初始化-3.png)
...
$$
Var[\frac{\partial Cost}{\partial w_{i}}] = [nVar(W)]^{d}Var[w]Var[\frac{\partial Cost}{\partial s^{d}}]
$$
...
3.  [深度前馈网络与Xavier初始化原理](https://zhuanlan.zhihu.com/p/27919794)
4.  [NMT Tutorial 3扩展c. 神经网络的初始化](http://txshi-mt.com/2018/11/16/NMT-Tutorial-3c-Neural-Networks-Initialization/)
{% endraw %}
