...
![](/img/yuque/计算机/知识图谱/知识图谱入门笔记系列/assets/7._知识推理-1.png)
...
![](/img/yuque/计算机/知识图谱/知识图谱入门笔记系列/assets/7._知识推理-2.png)

即若Mother 是 Women的子集，Women是 Person的子集，那么我们就可以得出 Mother是 Person的子集这个新类别关系。

实例化即计算属于某个概念或关系的所有**实例的集合**。如:

![](/img/yuque/计算机/知识图谱/知识图谱入门笔记系列/assets/7._知识推理-3.png)

第一个是计算新的类实例信息，首先已知Alice 是Mother，Mother 是 Women的子集，那么可知Alice 是一个Women。即为Women增加了一个新的实例。下面那个是计算新的二元关系，已知Alice 和Bob 有儿子，同时has\_son 是has\_child的子类，那么可知Alice 和Bob has\_child。
...
TBox为术语集，它是泛化的知识，是描述概念和关系的知识，被称之为公理(Axiom)。由于概念之间存在包含关系，TBox 知识形成类似格(Lattice)的结构，这种结构是由包含关系决定的，与具体实现无关。TBox语言有定义和包含，其中定义为引入概念及关系的名称，如Mother、Person、has\_child，包含指声明包含关系的公理，例如 $Mother \sqsubseteq \exists has_child.Person$

ABox是断言集，指具体个体的信息，ABox包含外延知识(又称为断言(Assertion)), 描述论域中的特定个体。**描述逻辑的知识库 K:= <T, A>， T即TBOx， A即ABOx。**ABox 语言包含概念断言和关系断言，概念断言即表示一个对象是否属于某个概念，例如Mother(Alice)、Person(Bob)。关系断言表示两个对象是否满足特定的关系，例如 has\_child(Alice, Bob)。
...
![](/img/yuque/计算机/知识图谱/知识图谱入门笔记系列/assets/7._知识推理-4.png)

因为OWL采用描述逻辑，因此下图给出了描述逻辑与OWL词汇的对应表：

![](/img/yuque/计算机/知识图谱/知识图谱入门笔记系列/assets/7._知识推理-5.png)
...
![](/img/yuque/计算机/知识图谱/知识图谱入门笔记系列/assets/7._知识推理-6.png)
...
![](/img/yuque/计算机/知识图谱/知识图谱入门笔记系列/assets/7._知识推理-7.png)
...
![](/img/yuque/计算机/知识图谱/知识图谱入门笔记系列/assets/7._知识推理-8.png)

Datalog 的基本语法包含:

-   原子(Atom): $p(t_1, t_2, \ldots, t_n)$， 其中p是谓词,n是目数, $t_i$ 是项 (变量或常量)，例如 has\_child(X, Y)；

-   规则(Rule)：$H :- B_1, B_2, \ldots, B_m$，由原子构建，其中H 是头部原子，$B_1, B_2, \ldots, B_m$是体部原子。例如:  has\_child X, Y : −has\_son X, Y

-   事实(Fact)： $F(c_1, c_2, \ldots, c_n ):-$ ，它是没有体部且没有变量的规则，例如  has\_child Alice, Bob : −

-   Datalog程序是规则的集合

下图给出一个Datalog 推理的例子：

![](/img/yuque/计算机/知识图谱/知识图谱入门笔记系列/assets/7._知识推理-9.png)

### 相关工具简介

![](/img/in-post/xiaoxiangkg_note7/xiaoxiangkg_note7_10.png)![](/img/yuque/计算机/知识图谱/知识图谱入门笔记系列/assets/7._知识推理-10.png)
...
![](/img/yuque/计算机/知识图谱/知识图谱入门笔记系列/assets/7._知识推理-11.png)

### 查询重写举例

查询所有研究人员及其所从事的项目? 用 SPARQL表述为:

![](/img/yuque/计算机/知识图谱/知识图谱入门笔记系列/assets/7._知识推理-12.png)

给定Datalog 规则如下:

![](/img/yuque/计算机/知识图谱/知识图谱入门笔记系列/assets/7._知识推理-13.png)

底层数据具体为某数据库中为下图中的两张表:

![](/img/yuque/计算机/知识图谱/知识图谱入门笔记系列/assets/7._知识推理-14.png)

-   步骤一： 重写为Datalog 查询

-   过滤不需要的公理 (通过语法层过滤)

![](/img/yuque/计算机/知识图谱/知识图谱入门笔记系列/assets/7._知识推理-15.png)

-   生成所有相关的Datalog 查询

![](/img/yuque/计算机/知识图谱/知识图谱入门笔记系列/assets/7._知识推理-16.png)

-   步骤二： 将数据库关系表达式映射成Datalog原子

![](/img/yuque/计算机/知识图谱/知识图谱入门笔记系列/assets/7._知识推理-17.png)

-   步骤三：将从SPARQL以及数据库重写过来的Datalog 规则整合进行查询  
    ![](/img/in-post/xiaoxiangkg_note7/xiaoxiangkg_note7_18.png) ![](/img/yuque/计算机/知识图谱/知识图谱入门笔记系列/assets/7._知识推理-18.png)
...
-   事实(Working Memory Element, WME)，包含描述对象和描述关系。描述对象形如$(type attr_1 : val_1 attr_2 : val_2 ... attr_n : val_n )$,其中type, attr\_i , val\_i 均为原子 (常量)， 例如 (student name: Alice age: 24)。描述关系(Refication)，例如 (basicFact relation: olderThan firstArg: John secondArg: Alice)简记为(olderThan John Alice)。
...
![](/img/yuque/计算机/知识图谱/知识图谱入门笔记系列/assets/7._知识推理-19.png)
...
![](/img/yuque/计算机/知识图谱/知识图谱入门笔记系列/assets/7._知识推理-20.png)
...
![](/img/yuque/计算机/知识图谱/知识图谱入门笔记系列/assets/7._知识推理-21.png)

# Ref

[小象](http://www.chinahadoop.cn/course/1048)
{% endraw %}
