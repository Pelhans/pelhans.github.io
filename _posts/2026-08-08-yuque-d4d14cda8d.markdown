...
1.  模型剪枝：即移除对结果作用较小的组件，如减少 head 的数量和去除作用较少的层，共享参数等，ALBERT属于这种
2.  量化：比如将 float32 讲到 float8
3.  蒸馏：将 teacher 的能力蒸馏到 student上，一般 student 会比 teacher 小。我们可以把一个大而深得网络蒸馏到一个小的网络，也可以把集成的网络蒸馏到一个网络上。
4.  参数共享：通过共享参数，达到减少网络参数的目的，如 ALBERT 共享了 Transformer 层
5.  参数矩阵近似：通过矩阵的低秩分解或其他方法达到降低矩阵参数的目的
...
![image.png](/img/yuque/计算机/深度学习/模型压缩/assets/模型压缩总结(至2020年初)-1.png)
...
![image.png](/img/yuque/计算机/深度学习/模型压缩/assets/模型压缩总结(至2020年初)-2.png)

那学生模型的深度到底影响有多大呢？论文分别作了不同深度/参数量的学生模型，如下图所示，可以看到，较深的学生模型在较低的参数量时，依旧可以获得比教师网络更好的效果：

![image.png](/img/yuque/计算机/深度学习/模型压缩/assets/模型压缩总结(至2020年初)-3.png)
...
![image.png](/img/yuque/计算机/深度学习/模型压缩/assets/模型压缩总结(至2020年初)-4.png)
...
-   最终 softmax 的 MSE loss： $||p(x) - q(x)||^{2}$

-   softmax 前的 logits MSE loss：$||l(x) - z(x)||^{2}$ 称之为 mimic loss

-   Hinton 的那个带温度的 loss

最终实验表明，第二个 mimic loss 效果最好。整个网络的回传梯度流如下图所示：

![image.png](/img/yuque/计算机/深度学习/模型压缩/assets/模型压缩总结(至2020年初)-5.png)

最终实验效果如下图所示，可以看出，这个方法比单纯的 KD 还是要好的：

![image.png](/img/yuque/计算机/深度学习/模型压缩/assets/模型压缩总结(至2020年初)-6.png)
...
![image.png](/img/yuque/计算机/深度学习/模型压缩/assets/模型压缩总结(至2020年初)-7.png)
...
![image.png](/img/yuque/计算机/深度学习/模型压缩/assets/模型压缩总结(至2020年初)-8.png)
...
![image.png](/img/yuque/计算机/深度学习/模型压缩/assets/模型压缩总结(至2020年初)-9.png)
...
![](/img/in-post/pretrain_model/route_kl.png)![image.png](/img/yuque/计算机/深度学习/模型压缩/assets/模型压缩总结(至2020年初)-10.png)

![image.png](/img/yuque/计算机/深度学习/模型压缩/assets/模型压缩总结(至2020年初)-11.png)

其中 H 表示 KL 散度，$\Chi^{'}$ 表示验证机，$r_{ij}$ 表示两个 epochs 间的 H 相对变化。若相对变化超过阈值(论文里对 MobileNetV2 用 0.8)，就切换 T。贪婪搜索算法的流程如下图所示：

![image.png](/img/yuque/计算机/深度学习/模型压缩/assets/模型压缩总结(至2020年初)-12.png)

最终模型的效果如下图所示，相比于传统 KD，改进还是比较大的。对于EEI 和  GS 方法的对比，当 EEI 为 4 时效果比 GS 没弱太多，one-stage EEI 效果也还好，但 EEI-2 和 EEI-3 不太行。

![image.png](/img/yuque/计算机/深度学习/模型压缩/assets/模型压缩总结(至2020年初)-13.png)
...
![image.png](/img/yuque/计算机/深度学习/模型压缩/assets/模型压缩总结(至2020年初)-14.png)

下图是两种策略的对比，可以看出 skip 要比 last 整体强一些。

![image.png](/img/yuque/计算机/深度学习/模型压缩/assets/模型压缩总结(至2020年初)-15.png)
...
![image.png](/img/yuque/计算机/深度学习/模型压缩/assets/模型压缩总结(至2020年初)-16.png)
...
![image.png](/img/yuque/计算机/深度学习/模型压缩/assets/模型压缩总结(至2020年初)-17.png)
...
其中 $H^{S}$ 的维度是 $l\times d^{'}$，$H^{T}$ 的维度是 $l\times d$，所以 $W_{h}$ 的维度是 $d^{'}\times d$。
...
![image.png](/img/yuque/计算机/深度学习/模型压缩/assets/模型压缩总结(至2020年初)-18.png)
...
![image.png](/img/yuque/计算机/深度学习/模型压缩/assets/模型压缩总结(至2020年初)-19.png)
...
![image.png](/img/yuque/计算机/深度学习/模型压缩/assets/模型压缩总结(至2020年初)-20.png)
...
![image.png](/img/yuque/计算机/深度学习/模型压缩/assets/模型压缩总结(至2020年初)-21.png)
...
![image.png](/img/yuque/计算机/深度学习/模型压缩/assets/模型压缩总结(至2020年初)-22.png)

论文还非常人性化的给了一个不同 BERT压缩方法的比较,如下图所示。“CE”和“MSE”分别代表交叉熵和均方误差。“KD”表示损失是为了知识的蒸馏。“CETASK”和“celm”分别表示对下游任务和 MLM 预训练计算的交叉熵：

![image.png](/img/yuque/计算机/深度学习/模型压缩/assets/模型压缩总结(至2020年初)-23.png)
...
![image.png](/img/yuque/计算机/深度学习/模型压缩/assets/模型压缩总结(至2020年初)-24.png)
...
-   减少各个transformer的 attention head数量

-   少各个transformer前馈子层的中间宽度
...
-   计算一个阈值，使 50% 的权重量级低于该阈值

-   移除那些权重

-   继续训练网络以恢复损失的准确性

-   （可选）返回到步骤 1 并增加修剪权重的百分比
...
![image.png](/img/yuque/计算机/深度学习/模型压缩/assets/模型压缩总结(至2020年初)-25.png)
{% endraw %}
