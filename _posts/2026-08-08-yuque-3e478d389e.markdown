...
![](/img/yuque/计算机/深度学习/深度学习基础/assets/15._Attention_及_Transformer_变体总结-1.png)
...
![](/img/yuque/计算机/深度学习/深度学习基础/assets/15._Attention_及_Transformer_变体总结-2.png)

上下文碎片化的问题解决了，但这又引入了另一个问题：如何保持位置信息的一致性？用公式来表述这个问题明显一点。令 segment 长度为 n

-   第 $\tau$ 个 segment 的输入 token 序列 $x_{\tau} = (x_{\tau, 1}, x_{\tau, 2},\dots, x_{\tau, n})$，对应的 embedding 矩阵 $E{x_{\tau}}$

-   第 $\tau + 1$ 个segment 的输入 token 序列 $x_{\tau+1} = (x_{\tau+1, 1}, x_{\tau+1, 2},\dots, x_{\tau+1, n})$，对应的 embedding 矩阵 $E{x_{\tau+1}}$
...
![](/img/yuque/计算机/深度学习/深度学习基础/assets/15._Attention_及_Transformer_变体总结-3.png)
...
![](/img/yuque/计算机/深度学习/深度学习基础/assets/15._Attention_及_Transformer_变体总结-4.png)
...
详细来说，这个 递归机制如下图所示，其中比如输入是 $h_{1}^{t}$，正常的 Transformer 就是经过 self-attention + ADD\_AND\_LN +  FFN + ADD\_AND\_LN 就进入下一层了，但这里不是，经过 self-attnetion 后还要进入一个叫做  Transition function 的模块，这个模块是共享权重的，具体形式上来说， Transition function 可以使之前一样的 FFN，也可以是 可分离卷积层。

![](/img/yuque/计算机/深度学习/深度学习基础/assets/15._Attention_及_Transformer_变体总结-5.png)
...
![](/img/yuque/计算机/深度学习/深度学习基础/assets/15._Attention_及_Transformer_变体总结-6.png)
...
![](/img/yuque/计算机/深度学习/深度学习基础/assets/15._Attention_及_Transformer_变体总结-7.png)
...
![](/img/yuque/计算机/深度学习/深度学习基础/assets/15._Attention_及_Transformer_变体总结-8.jpeg)
...
![](/img/yuque/计算机/深度学习/深度学习基础/assets/15._Attention_及_Transformer_变体总结-9.jpeg)
...
![](/img/yuque/计算机/深度学习/深度学习基础/assets/15._Attention_及_Transformer_变体总结-10.jpeg)

## Generating Long Sequences with Sparse Transformers

论文中将 full attention 进行分解，通过多个 sparse attention 来代替，在不牺牲性能的情况下降低复杂度至 $O(N\sqrt(N))$，如下图所示：

![](/img/yuque/计算机/深度学习/深度学习基础/assets/15._Attention_及_Transformer_变体总结-11.png)
...
![](/img/yuque/计算机/深度学习/深度学习基础/assets/15._Attention_及_Transformer_变体总结-12.png)
...
![](/img/yuque/计算机/深度学习/深度学习基础/assets/15._Attention_及_Transformer_变体总结-13.png)
...
![](/img/yuque/计算机/深度学习/深度学习基础/assets/15._Attention_及_Transformer_变体总结-14.png)
...
![](/img/yuque/计算机/深度学习/深度学习基础/assets/15._Attention_及_Transformer_变体总结-15.png)
...
![](/img/yuque/计算机/深度学习/深度学习基础/assets/15._Attention_及_Transformer_变体总结-16.png)

![](/img/yuque/计算机/深度学习/深度学习基础/assets/15._Attention_及_Transformer_变体总结-17.png)
...
![](/img/yuque/计算机/深度学习/深度学习基础/assets/15._Attention_及_Transformer_变体总结-18.png)
...
![](/img/yuque/计算机/深度学习/深度学习基础/assets/15._Attention_及_Transformer_变体总结-19.png)
...
![](/img/yuque/计算机/深度学习/深度学习基础/assets/15._Attention_及_Transformer_变体总结-20.png)
...
![](/img/yuque/计算机/深度学习/深度学习基础/assets/15._Attention_及_Transformer_变体总结-21.png)

实验结果如下图所示，在 text8 和 enwik8 数据集上，大模型和小模型与其他模型的对比在小的模型上，long former比其他模型都要好在大的模型上，比18层的transformerxl要好，跟第二个和第三个齐平

![](/img/yuque/计算机/深度学习/深度学习基础/assets/15._Attention_及_Transformer_变体总结-22.png)

递增窗口大小的表现最好，递减窗口大小表现最差，使用固定窗口的表现介于两者之间。不用膨胀比对2个头膨胀效果差一点

![](/img/yuque/计算机/深度学习/深度学习基础/assets/15._Attention_及_Transformer_变体总结-23.png)
...
第一种被称为 Dense 的结构，既然不要 token 对 token 这种形式，那是否可以考虑移除掉 query 或者 key 中的一个呢？我们的输入 X 的维度是 $l \times d$，其中 l 是输入序列的长度，则 B 的维度应该是 $l \times l$。如果只用 query 或者 key 中的一个的话，那么我们可以让 X 乘以一个 $d \times l$ 的矩阵 W 就可以了，即 $B = XW$。当然这样有点太简单了，因此论文中实际采用的是：

$$
B = W(ReLU(W(X) + b)) + b
$$

这个还是可以理解的形式，毕竟之前也有人用多层感知机形式的计算 attention。但下面的这个 random 的就。。。刚刚不是说 B 的 shape 是 $l \times l$ 的嘛，那我们可不可以直接初始化一个 shape 为 $l\times l$ 的 random 矩阵呢？论文就这么做了，并称之为 Random 结构。

模型结构如下图所示：

![](/img/yuque/计算机/深度学习/深度学习基础/assets/15._Attention_及_Transformer_变体总结-24.png)

除了上面两个之外，论文还讨论了低秩分解的问题，想通过低秩分解来降低参数量，对于 Dense 和 Random 的对应低秩分解形式论文称之为 Factorized Dense 和 Factorized Random。

对于 Dense 结构，低秩分解的方式就是生成两个 $l \times a$ 和 $l \times b$的矩阵 $B{1}$ *和* $B{2}$，其中  $a*b=l$，而后将  $B_{1}$ 重复 b次， $B{2}$ *重复 a 次，得到对应的*  $l\times l$ *矩阵* $\tilde{B}{1}, ~~\tilde{B}{2}$*，然后逐位相乘，得到* $l\times l$ *矩阵 B。对于 Random 的低秩形式，是去两个 $$ lxk$$ 的矩阵* $R_{1}$ 和 $R_{2}$*，令它们相乘即可得到* $b = R_{1}R_{2}^{T}$

论文分别再翻译、摘要。对话任务上进行了评测。翻译任务上的结果如下图所示：

![](/img/yuque/计算机/深度学习/深度学习基础/assets/15._Attention_及_Transformer_变体总结-25.png)
...
![](/img/yuque/计算机/深度学习/深度学习基础/assets/15._Attention_及_Transformer_变体总结-26.png)

在预训练模型上论文也做了实验，不过在这里 Dense 和 Random就不大行了。

![](/img/yuque/计算机/深度学习/深度学习基础/assets/15._Attention_及_Transformer_变体总结-27.png)

坐着对比了训练 5 万步后的参数分布，对比的是 Encoder 层的 1,3,5,6 层，得出它们的参数分布大致相同的结论。

![](/img/yuque/计算机/深度学习/深度学习基础/assets/15._Attention_及_Transformer_变体总结-28.png)
...
![](/img/yuque/计算机/深度学习/深度学习基础/assets/15._Attention_及_Transformer_变体总结-29.png)
...
![](/img/yuque/计算机/深度学习/深度学习基础/assets/15._Attention_及_Transformer_变体总结-30.png)

消融实验证明,添加的两个蒸馏损失项能起到弥补精度损失的效果。

![](/img/yuque/计算机/深度学习/深度学习基础/assets/15._Attention_及_Transformer_变体总结-31.png)
...
现在我们知道 QKV 都是 nxd 的，之前先算 $QK^{T}$ 的话，得到 $n\times n$ 的，此时需要计算 $n^{2}d$ 次，之后这个 nxn 的再和 V 相乘需要计算 $n^{2}d$ 次，整体来说复杂度就是 $O(n^{2}d)$。而现在先算 $K^{T}V$ 的话，需要计算 $d^{2}n$ 次，这个再与 Q 乘也需要 $nd^{2}$ 次，整体复杂度变成了  $O(nd^{2})$，也就是序列长度的线性复杂度。当 d 比 n小时，这就可以大大的降低运算的复杂度。
...
论文提出的线性 Transformer 模型结合了两者的优点。当涉及到训练时，计 算可以并行化，并充分利用gpu或其他加速器。当涉及到推理时，对于论文中的 模型，每次预测的每次成本和内存都是恒定的。这意味着我们可以简单地将 φ（Kj）V\_j^T矩阵存储为内部状态，并像递归神经网络一样在每一个时间步 对其进行更新。这导致推断速度比其他 Transformer 模型快数千倍。
...
![](/img/yuque/计算机/深度学习/深度学习基础/assets/15._Attention_及_Transformer_变体总结-32.png)
...
![](/img/yuque/计算机/深度学习/深度学习基础/assets/15._Attention_及_Transformer_变体总结-33.png)
...
现在引入两个映射矩阵 $E_{i},~F_{i}\in R^{n\times k}$，你 $KW_{i}^{K}$ 的维度不是 nxd 么？那我再乘一个 E ，就会得到一个 kxd 的矩阵，对 K 和 V 都做这种事，就得到了下式：
...
![](/img/yuque/计算机/深度学习/深度学习基础/assets/15._Attention_及_Transformer_变体总结-34.png)
...
![](/img/yuque/计算机/深度学习/深度学习基础/assets/15._Attention_及_Transformer_变体总结-35.png)
...
![](/img/yuque/计算机/深度学习/深度学习基础/assets/15._Attention_及_Transformer_变体总结-36.png)
...
![](/img/yuque/计算机/深度学习/深度学习基础/assets/15._Attention_及_Transformer_变体总结-37.png)
...
![](/img/yuque/计算机/深度学习/深度学习基础/assets/15._Attention_及_Transformer_变体总结-38.png)

![](/img/in-post/various_attention/bigbird_res2.png)  
![](/img/in-post/various_attention/bigbird_res3.png)![](/img/yuque/计算机/深度学习/深度学习基础/assets/15._Attention_及_Transformer_变体总结-39.png)
{% endraw %}
