...
**正则化的本质就是对参数的先验假设**。通过对参数的正则化，以偏差的增加换取方差的减少，从而使得机器学习算法的泛化性增加。

-   **偏差是指模型的预测值与真实值之间的差异，即模型在训练数据上的系统误差。高偏差通常意味着模型过于简单（欠拟合），不能捕捉到数据的真实关系，即模型的预测值与真实值之间存在较大的系统性误差。**
-   **方差是指模型对于给定点的预测值在不同训练数据集上的波动程度。高方差意味着模型对训练数据非常敏感，即模型在不同的数据集上可能会有很大的预测差异，这通常是过拟合的表现。**

因此高偏差相当于模型欠拟合，而高方差是过拟合，导致泛化能力弱。大多数机器学习算法则是在偏差-方差，经验风险-结构风险之间做权衡。将正则项 $\Omega(\theta)$加入目标函数 J，有：
...
![](/img/yuque/计算机/深度学习/深度学习基础/assets/6._正则化项-1.png)
...
我们可以按照 L2 正则分析的逻辑来看 L1. 对模型参数 w 的 L1 正则化被定义为：
...
可以看到，简化后得到 $\lambda|w|_{1}$，这就是 L1 正则化的形式。

![](/img/yuque/计算机/深度学习/深度学习基础/assets/6._正则化项-2.jpeg)
...
![](/img/yuque/计算机/深度学习/深度学习基础/assets/6._正则化项-3.png)
