...
要将异常串联起来，可以用 raise from 语句来代替普通的 raise。还可以通过查看异常对象的 **cause**属性来跟踪所希望的异常链。
...
现在要构建出目标库，只需要用 python3 buildlib.py build\_ext --inplace即可。这样就创建了一个名为 sample.so 的共享库。编译结束后，应该就可以开始将其当做一个Python模块来导入了。
...
swig会产生两个文件：sample\_wrap.c 和 sample.py。而后编写 setup.py.py 并进行编译即可正常导入。
